...
RooQuiz 提供一个 **MCP(Model Context Protocol)server**,让 Claude Desktop、Claude Code、Cursor、Codex 等 AI 客户端能在你的团队下直接操作测验——用自然语言就能创建考试、评分诊断、分型测评,追加题目、调整顺序、配置报告维度与分型结果;还能复制 / 删除测验、做多语言译文、管理考生,以及读取答题记录、统计与转化漏斗——而不必手动在编辑器里一个个搭。
...
| 工具 | 作用 |
| --- | --- |
| `create_form` | 创建测验(`quiz` 考试、`scorecard` 评分诊断 或 `outcome` 分型测评),可一次性传入题目数组和报告配置,减少往返调用。分型测评**必须**在创建时一并传入 `report.outcomes`(结果类型列表),并让每个选项通过 `outcomes` 给类型投票。 |
...
<Callout type="info">
  典型流程:
  - **考试 / 评分诊断**:`create_form` 创建测验并拿到每道题的 `code` → 用 `update_form` 或 `set_dimension_analysis` 按 `code` 配置报告公式 / 维度。
...
- **评分诊断(scorecard)的选项计分**走「选项分值(Option Scoring)」:`update_question` 在 scorecard 场景下不接受 `score` / `correctAnswer` / `aiMatch`,改选项分值请在后台编辑,或删了重建。分型测评(outcome)同样不接受这三个参数(它按票数判型,没有计分概念)。
