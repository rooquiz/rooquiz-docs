...
| Query param | Type | Default | Description |
| --- | --- | --- | --- |
| `formId` | string | — | Only submissions for this form. |
| `status` | string | — | Report status: `pending`, `processing`, `completed`, `failed`. |
| `since` | ISO date | — | Only submissions created at/after this time (incremental sync). |
| `until` | ISO date | — | Only submissions created at/before this time. |
| `limit` | number | `20` | Items per page (max `100`). |
| `page` | number | `1` | Page number. |
